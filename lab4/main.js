...
function scheduleAudio() {
    ...
    liveCodeState.forEach(noteData => {
        ...
        console.log(noteData["osc"])
        osc.type = noteData["osc"]
        osc.frequency.setTargetAtTime(((noteData["pitch"] < 0)?(Math.floor(Math.random() * noteData["pitch"])):(noteData["pitch"])), audioCtx.currentTime + timeElapsedSecs, 0.01)
